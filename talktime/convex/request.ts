...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const receiver = await ctx.db.query("users").withIndex("byEmail", (q) => q.eq("email", args.email)).unique();
        ...
        if (friends1.some((friend) => friend.user2 === receiver._id) || friends2.some((friend) => friend.user1 === receiver._id)) {
            ...
            // This check is to prevent sending a friend request to someone who is already a friend
        }
...
export const deny = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const requests = await ctx.db.get(args.id)

        if (!requests || requests.receiver !== currentUser._id) {
            throw new ConvexError("There was an error denying this request");
        }

        await ctx.db.delete(requests._id);
    },
