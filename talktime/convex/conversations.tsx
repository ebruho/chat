...
export const get = query({
    ...
    handler: async (ctx, args) => {
        ...
        const conversationsWithDetails = await Promise.all(conversations.map(async (conversations, index) => {
            const allConversationMemberships = await ctx.db
            .query("conversationMembers").withIndex("by_conversationId", (q) => 
                q.eq("conversationId", conversations?._id)).collect();

            const lastMessage = await getLastMessageDetails({ctx, id: conversations.lastMessageId})
            ...
            const unseenMessages = await ctx.db.query("messages").withIndex("by_conversationId", (q) =>
                q.eq("conversationId", conversations._id)).filter((q) => q.gt(q.field("_creationTime"), lastSeenMessageTime))
            .filter((q) => q.neq(q.field("senderId"), currentUser._id)).collect();

            if (conversations.isGroup){
                return {conversations, lastMessage, unseenCount: unseenMessages.length};
            }else{
